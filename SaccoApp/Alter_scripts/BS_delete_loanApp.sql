...
/****** Object:  StoredProcedure [dbo].[BS_delete_loanApp]    Script Date: 12/16/2017 9:57:00 AM ******/
SET ANSI_NULLS ON
GO
...
create procedure dbo.BS_delete_loanApp(@LoanNo    nvarchar(45),
...
begin try
	...
	begin try
		DELETE FROM dbo.BS_LOANGUAR WHERE LoanNo = @LoanNo
	end try
	begin catch
		set @v_error = 'Unable to delete loan guarantors...' + ERROR_MESSAGE();
		RAISERROR (@v_error, 16,1)
	end catch

	begin try
		DELETE FROM dbo.BS_LOANS
		WHERE LoanNo = @LoanNo AND MemberNo = @MemberNo
	end try
	begin catch
		set @v_error = 'Unable to delete loan...' + ERROR_MESSAGE();
		RAISERROR (@v_error, 16,1)
	end catch
	COMMIT
end try
